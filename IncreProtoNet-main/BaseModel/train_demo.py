...
def main():
    ...
    parser.add_argument("--base_val_file", default="data/fewrel/base_val_fewrel.json", help="base data val path") # Originally this was not present
    ...
    val_data_file = path_util.from_project_root(opt.base_val_file)
    ...
    if embedding_type == "glove":
        ...
        sentence_encoder = BERTSentenceEncoder(pretrain_ckpt, max_length, int(opt.word_embedding_dim), opt.cnn_hidden_size,
    ...
    val_data_loader = get_loader(val_data_file, baserel2index_file, sentence_encoder, batch_size)
    test_data_loader = get_loader(test_data_file, baserel2index_file, sentence_encoder, batch_size)

    """
        搭建framework
    """
    framework = BaseREFramework(train_data_loader, val_data_loader, test_data_loader)
    ...
    framework.eval(model, opt.val_iter, ckpt=ckpt)
