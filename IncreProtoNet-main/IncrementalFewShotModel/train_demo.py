...
def main():
    ...
    parser.add_argument("--test_iter", default=1000, type=int, help="test iterations")
    ...
    parser.add_argument("--basemodel_ckpt", default="../checkpoint/", help="base pretrained file")
    ...
    parser.add_argument("--val_baseData_path", default="data/fewrel/base_val_fewrel.json", help="base testing data path")
    parser.add_argument("--val_novelData_path", default="data/fewrel/val_novel_fewrel.json", help="novel testing data path")
    ...
    # Loading entire model checkpoints
    parser.add_argument("--load_ckpt", default=None, help="path to the model")
    parser.add_argument("--only_test", action="store_true", help="skip training of model")
    
    opt = parser.parse_args()
    baseRel2index_path = from_project_root(opt.baserel2index_path)
    train_data_path = from_project_root(opt.train_data_path)
    val_baseData_path = from_project_root(opt.val_baseData_path)
    val_novelData_path = from_project_root(opt.val_novelData_path)
    ...
    basemodel, sentence_encoder = load_pretrain_model(baseModel_ckpt, opt.embedding_type, opt.max_length, opt.cnn_hidden_size,
                                   int(opt.word_embedding_dim), opt.pos_embedding_dim, opt.baseNumRels, opt.pl_weight, is_simQ)
    ...
    if opt.embedding_type == "glove":
        ...
        meta_cnn_encoder = MetaCNN(opt.max_length, word_embedding_dim=int(opt.word_embedding_dim), pos_embedding_dim=0,
                                      hidden_size=opt.cnn_hidden_size)
    ...
    train_data_loader = train_get_loader(train_data_path, baseRel2index_path, sentence_encoder, opt.batch_size, opt.train_baseN,
                                   opt.train_novelN, opt.K, opt.train_Q, opt.triplet_num, opt.bi_pos_num, opt.bi_neg_num, num_workers=0)
    val_data_loader = test_get_loader(val_baseData_path, val_novelData_path, baseRel2index_path, sentence_encoder,
                                   opt.batch_size, opt.test_baseN, opt.test_novelN, opt.K, opt.test_Q, opt.bi_pos_num, opt.bi_neg_num, num_workers=0)
    ...
    framework = IncreFewShotREFramework(train_data_loader, val_data_loader, test_data_loader)
    ...
    framework.eval(model, opt.test_iter, opt.K, opt.cnn_hidden_size, opt.test_baseN, opt.test_novelN, opt.test_Q, biQ, test=True, ckpt=opt.load_ckpt)

    # Perform testing across all settings
    for N in [5, 10]:
        for K in [1, 5]:
            # Note the IncreProtoNet needs at least one Novel class to function, so it is tested using 1 novel class and N-1 base classes
            test_data_loader = test_get_loader(test_baseData_path, test_novelData_path, baseRel2index_path, sentence_encoder,
                                   opt.batch_size, N-1, 1, opt.K, opt.test_Q, opt.bi_pos_num, opt.bi_neg_num, num_workers=0)
            framework.test_data_loader = test_data_loader
            
            print("Calculating accuracy for {}-{} problem".format(N, K))
            framework.eval(model, opt.test_iter, opt.K, opt.cnn_hidden_size, N-1, 1, opt.test_Q, biQ, test=True, ckpt=opt.load_ckpt)
...
def load_pretrain_model(base_ckpt, embedding_type, max_length, cnn_hidden_size, word_embedding_dim,
    ...
    if embedding_type == "glove":
        ...
        pretrain_ckpt = "pretrain/TinyBERT_incremental_KD_best" #'bert-base-uncased'
        if is_simQ:
