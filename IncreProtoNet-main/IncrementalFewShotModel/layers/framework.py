...
class IncreFewShotREFramework:
    ...
    def __init__(self,train_data_loader, val_data_loader, test_data_loader):
        print("Using IncreFewShotREFramework")
    ...
    def train(self, model, bert_optim, learning_rate, train_iter, val_step, val_iter, save_ckpt, K, hidden_size,
        ...
        for it in range(start_iter, start_iter + train_iter):
            ...
            # 验证模型
            if (it + 1) % val_step == 0:
                base_acc, novel_acc, both_acc = self.eval(model, val_iter, K, hidden_size, test_baseN, test_novelN, test_Q, biQ, test=False)
                model.train()
    ...
    def eval(self, model, val_iter, K, hidden_size, test_baseN, test_novelN, test_Q, biQ, test=False, ckpt=None):
        print("")
        ...
        with torch.no_grad():
            for it in range(val_iter):
                ...
                novelSupport_set, query_set, query_label, base_label = next(eval_data_loader)
