...
class Mtb(fewshot_re_kit.framework.FewShotREModel):
    ...
    def forward(self, support, query, N, K, total_Q, is_student=False, is_teacher=False):
        ...
        support = support_output["encoding"]  # (B * N * K, D), where D is the hidden size
        query = query_output["encoding"] # (B * total_Q, D)
        ...
        if is_student or is_teacher:
            return combined_logits, pred, support_output, query_output
