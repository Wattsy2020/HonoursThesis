...
class TinyBertForSequenceClassification(BertPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None,
        ...
        '''Inputs: input_ids, token_type_ids (a list, 0 means first sentence 1 means second), attention_mask, labels,
        is_student (if true performs the transformation on it's outputs needed for KD)'''
    ...
    # because this inherits from BertPreTrainedModel it also has a .from_pretrained() method, that loads in the weights 
    # However this doesn't load in the classifier weights, as that varies depending on the number of classes for each specific task so can't be pretrained generally

# TinyBERT without the Sequence classification head, for use as an encoder in meta-learning models
class TinyBert(BertPreTrainedModel):
    def __init__(self, config, fit_size=768):
        ...
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.fit_dense = nn.Linear(config.hidden_size, fit_size)
        self.apply(self.init_bert_weights) # this is defined in BertPreTrainedModel and separately initialises the layernorm and linear layers
                                           # self.apply is defined by torch.nn.Module and applies a function recursively to each module https://pytorch.org/docs/stable/generated/torch.nn.Module.html 
